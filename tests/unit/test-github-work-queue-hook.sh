...
# Port adaptation: the hook only fires when the checkout's git remote matches
# the upstream repo (it is a maintainer tool for nyldn/claude-octopus). This
# repo's origin is the dsh port, so register a temporary upstream remote for
# the duration of the hook executions and remove it afterwards. The hook itself
# is byte-identical to upstream.
_temp_remote="octo-test-upstream"
git -C "$PROJECT_ROOT" remote add "$_temp_remote" https://github.com/nyldn/claude-octopus.git 2>/dev/null || true
trap 'git -C "$PROJECT_ROOT" remote remove "$_temp_remote" 2>/dev/null || true' EXIT
...
git -C "$PROJECT_ROOT" remote remove "$_temp_remote" 2>/dev/null || true
trap - EXIT
